...
1. Open `package.json`, look for and update the following things:
    ...
    1. Find and replace `atopile compiler` with display name for your tool. This is used as the title for the extension in market place, extensions view, output logs, etc. For example, for the `black` extension this is `Black Formatter`.
...
After finishing the getting started part, this template would have added the following. Assume `atopile` was replaced with `mytool`, and `atopile compiler` with`My Tool`:
